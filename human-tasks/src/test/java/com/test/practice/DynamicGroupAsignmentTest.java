 ...
 * 
 */
public class DynamicGroupAsignmentTest extends BaseHumanTaskTest {
	...
	protected String[] getProcessPaths() {
		return new String[] { "com/test/practice/03-dynamic-group-asignment.bpmn" };
	}
	...
	public void completeSimpleHumanTask() throws InterruptedException {
		...
		// now we have to pass the "group" variable, with the group id we want to use.
		inputVars.put("group", "hr");
		ProcessInstance process = session.createProcessInstance(
				"DynamicGroupAsignment", inputVars);
